...
def main ():
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument("-r", "--root", type=str, default="/home/lsi/UC3M/Simuladores_de_Robots/P3_Gazebo/gazebo-tools-master/assets", help="path to folder map")
    parser.add_argument("--map", type=int, default=1, help="number of map")
    parser.add_argument("--start_x", type=int, default=1, help="start coordinate x")
    ...
    # FILE_NAME = os.path.join (args.root, "map" + str(args.map) + ".csv")
    FILE_NAME = os.path.join ("assets/map" + str(args.map) + ".csv")
    ...
    # numpy_array = np.array(charMap)
    # transpose = numpy_array.T
    # charMap = transpose.tolist()
    ...
    print('%%%%%%%%%%%%%%%%%%%')
    ok = False
    output_path = ALGORITHM + '_points.txt'
    with open (output_path, 'w') as output_file:
        output_file.write ( str (END_X) + ' ' + str (END_Y) + '\n' ) # Write goal point
        while not ok:
            for node in nodes:
                if( node.myId == goal_pared_id_aux ):
                    node.dump()
                    output_file.write ( str (node.x) + ' ' + str (node.y) + '\n' )
